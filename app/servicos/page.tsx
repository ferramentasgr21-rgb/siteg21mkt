...
const COMERCIAL = [
  ...
  { icon: Cpu, title: 'Ferramentas e tecnologia de vendas', result: 'Controle total da operação com tecnologia de ponta.', items: ['Análise e implantação de CRM', 'Suporte na implantação de ERP', 'Atuomação do atendimento', 'Tabela de vendas e prazos'] },
...
const MARKETING = [
  { icon: Palette, title: 'Branding e posicionamento', result: 'Marca forte, diferenciada e memorável no mercado.', items: ['Identidade visual da marca', 'Identidade visual do produto', 'Posicionamento estratégico', 'Diagnóstico e análise SWOT', 'Análise da concorrência', 'NPS e pesquisa de satisfação'] },
  { icon: LineChart, title: 'Planejamento estratégico de marketing', result: 'Marketing estratégico, mensurável e com ROI comprovado.', items: ['Estruturação 100% da área de marketing', 'Jornada do cliente', 'Planejamento e execução de lançamento', 'Orçamento anual de marketing', 'Indicadores (ROI, CAC, CPA)'] },
  { icon: Radio, title: 'Comunicação 360°', result: 'Presença digital forte e leads qualificados entrando.', items: ['Set-up completo nos canais', 'Mídia própria (site, redes, blog)', 'Mídia paga (Google, Meta, portais)', 'Assessoria de imprensa', 'Relações Públicas' , 'Criativos ilimitados e landing pages'] },
...
export default function ServicosPage() {
  return (
    <>
      <PageHero eyebrow="O que oferecemos" title="Soluções completas de vendas e marketing para a sua incorporadora" subtitle="Da estratégia à execução, oferecemos um ecossistema integrado de serviços comerciais e de marketing, desenhado especificamente para o mercado imobiliário."/>
  ...
  {COMERCIAL?.map((s, i) => (
    <div key={s?.title} className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)]">
      ...
      <section className="bg-brand-cream py-20 md:py-24">
        <div className="mx-auto max-w-[1200px] px-5">
          <SectionHeading eyebrow="Área de marketing" title="Marketing imobiliário estratégico" subtitle="Da construção da marca àao relacionamento contínuo." />
          <div className="mt-14 flex flex-wrap justify-center gap-6">
  {MARKETING?.map((s, i) => (
    <div key={s?.title} className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)]">
      <ServiceCard {...s} i={i} />
      ...
      <section className="py-20 md:py-24">
        <div className="mx-auto max-w-[1200px] px-5">
          <SectionHeading eyebrow="Modelo de atuação" title="Flexibilidade para atender sua incorporadora" />
      ...
      <CtaBand title="Qual solução faz sentido para sua incorporadora?" subtitle="Agende um diagnóstico e vamos desenhar a estratégia ideal para você." primaryLabel="Agendar Diagnóstico" />
    </>
  );
}
